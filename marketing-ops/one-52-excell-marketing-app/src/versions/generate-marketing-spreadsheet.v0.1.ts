...
import type { Fill, Alignment } from 'exceljs';
...
async function generateMarketingSpreadsheet(): Promise<void> {
  ...
  // Add app parameters
  appParameters.forEach(([name, value, unit, note], _index) => {
  ...
  months.forEach((month, _index) => {
    const row = appGrowthWs.addRow({
      month: month,
      baseRevenue: _index === 0 ? 8000 : `=F${_index + 2}`,
      recipients: `='ONE52 Bar App'!B5*(1-'ONE52 Bar App'!B7)`,
      newCustomers: `=C${_index + 2}*'ONE52 Bar App'!B8*2`,
      newRevenue: `=D${_index + 2}*'ONE52 Bar App'!B8`,
      totalRevenue: `=B${_index + 2}+E${_index + 2}`,
      marketingCost: `=C${_index + 2}*'ONE52 Bar App'!B4+'ONE52 Bar App'!B16+IF(E${_index + 2}>8000,MAX(0,E${_index + 2}-8000)*'ONE52 Bar App'!B18,0)`,
      curbsideRevenue: `=D${_index + 2}*'ONE52 Bar App'!B8*'ONE52 Bar App'!B19`,
      cookCost: `='ONE52 Bar App'!B20*'ONE52 Bar App'!B21*'ONE52 Bar App'!B22*4`,
      netRevenue: `=F${_index + 2}+H${_index + 2}-I${_index + 2}-G${_index + 2}`
    });
  ...
  // Save the workbook
  const outputPath = path.join(process.cwd(), 'dist', 'marketing-campaign.xlsx');
