...
function checkPrismaImports(content) {
...
function checkComponentActionImports(filePath, componentType) {
 ...
 */
export async function registerTapPassMember(formData) {
  try {
    const name = formData.get('name');
    const email = formData.get('email');
    const birthday = formData.get('birthday');
 ...
 */
export async function emailMembershipCard(formData) {
  try {
    const email = formData.get('email');
...
async function checkPrismaMigration() {
